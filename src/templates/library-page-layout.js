...
const installText = (name, type = "npm") =>
  type === "npm" ? `npm i ${name} --save` : `yarn add ${name}`
      ...
      <div className="mx-auto w-full md:w-3/5">
        ...
        {tags && (
          <div className="flex my-5">
            {tags.map((tag, i) => (
              <span key={i} className="pill mr-1 mb-1 uppercase text-xs">
                {tag}
              </span>
            ))}
          </div>
        ...
        {description && (
          <p className="py-4 pl-6 border-l-4 border-purple-500 bg-gray-800">
            {description}
          </p>
        )}
        {install && (
