...
const LibraryCard = ({ slug, title, description, demo, github, npm }) => (
  <div className="w-full md:w-1/4 px-2 mb-5">
    <div className="h-full p-6 pb-16 rounded bg-gray-800 relative">
      ...
      {
        (demo || github) && (
          <div className="absolute px-5 left-0 bottom-0 pb-5 w-full flex flex-row text-purple-500">
            <div className="flex flex-grow">
              ...
              { npm && (
                <a href={npm} className="mr-3" target="_blank" rel="noopener noreferrer">
                  <Download />
                </a>
              ) }
            </div>
            <div>
              <Link to={`${slug}`}><Eye /></Link>
            </div>
          </div>
        )
      }
